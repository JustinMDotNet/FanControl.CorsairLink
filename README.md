...
| `FANCONTROL_CORSAIRLINK_ICUELINK_LIGHTING` <sup>4</sup>                   | Enables software-driven flowing-gradient lighting on the iCUE LINK Hub so LEDs stay lit after the hub is switched to software-controlled mode. | `1` = enabled, `0` = disabled |
| `FANCONTROL_CORSAIRLINK_ICUELINK_LIGHTING_COLORS` <sup>4</sup>            | The palette the gradient flows through (all colors shown at once, scrolling). Defaults to a 4-color Watercolor-style palette. | pipe-separated `R,G,B` colors, e.g. `255,0,0\|0,255,0\|0,0,255` |
...
| `FANCONTROL_CORSAIRLINK_ICUELINK_LIGHTING_CYCLE_SECONDS` <sup>4</sup>     | Seconds for the gradient to scroll one full loop. Defaults to `12`. | seconds, e.g. `20`                        |
