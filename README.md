...
| `FANCONTROL_CORSAIRLINK_ICUELINK_LIGHTING` <sup>4</sup>                   | Enables the software "Watercolor" lighting effect on the iCUE LINK Hub so LEDs stay lit after the hub is switched to software-controlled mode. | `1` = enabled, `0` = disabled |
...
| `FANCONTROL_CORSAIRLINK_ICUELINK_LIGHTING_CYCLE_SECONDS` <sup>4</sup>     | Seconds for the effect to scroll one full color cycle. Defaults to `4`. | seconds, e.g. `6`                  |
